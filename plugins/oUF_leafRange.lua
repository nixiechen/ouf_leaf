...
local function Enable(self)
	if self.leafRange and (self.unit ~= 'player') then
		UpdateRate = (type(self.leafRange) == 'number') and self.leafRange or UpdateRate
		Objects[self] = true
		return true
	end
end
...
oUF:AddElement('leafRange', UpdateRange, Enable, Disable)
